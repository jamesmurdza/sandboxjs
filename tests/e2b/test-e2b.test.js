...
describe('E2B Sandbox Operations', () => {
  ...
  // Create
  test('create E2B sandbox', async () => {
    sandbox = await Sandbox.create('e2b');
  ...
  test('update E2B sandbox (suspend/resume)', async () => {
    sandbox = await Sandbox.create('e2b');
  ...
  test('write via runCommand and read via connected sandbox', async () => {
    ...
    const content = 'Hello from E2B sandbox!';

    await sandbox.runCommand(`echo '${content}' > ${filename}`);

    const sandbox2 = await Sandbox.connect('e2b', sandbox.id());
  ...
  test('clean up directory after operations', async () => {
    ...
    // Verify cleanup
    const rootAfterCleanup = await sandbox.listFiles('.');
    expect(rootAfterCleanup.some(({ name }) => name === dirname)).toBe(false);
  }, 20000);

  // Delete sandbox
  test('delete E2B sandbox', async () => {
    sandbox = await Sandbox.create('e2b');
