...
describe('Daytona Sandbox Operations', () => {
  let sandbox;
  ...
  // Create
  test('create Daytona sandbox', async () => {
    sandbox = await Sandbox.create('daytona');
  ...
  test('update Daytona sandbox (suspend/resume)', async () => {
    sandbox = await Sandbox.create('daytona');
  ...
  test('write via runCommand and read via connected sandbox', async () => {
    ...
    const content = 'Hello from Daytona sandbox!';

    await sandbox.runCommand(`echo '${content}' > ${filename}`);

    const sandbox2 = await Sandbox.connect('daytona', sandbox.id());
  ...
  test('clean up directory after operations', async () => {
    sandbox = await Sandbox.create('daytona');

    const filename = 'cleanup_test.txt';
    const dirname = 'cleanup_dir';
    const content = 'Hello from Daytona sandbox!';
    ...
    // Clean up: delete file only (Daytona doesn't support directory deletion)
    await sandbox.deleteFile(newPath);

    // Verify file cleanup (directory will remain as Daytona limitation)
    const dirAfterCleanup = await sandbox.listFiles(dirname);
    expect(dirAfterCleanup.some(({ name }) => name === `moved_${filename}`)).toBe(false);
  }, 20000);

  // Delete sandbox
  test('delete Daytona sandbox', async () => {
    sandbox = await Sandbox.create('daytona');
